...
import org.apache.spark.network.client.{RpcResponseCallback, TransportClientBootstrap, NettyUcxTransportClient, NettyUcxTransportClientFactory}
...
class NettyUcxBlockTransferService (
  ...
  extends NettyBlockTransferService(
    conf, securityManager, bindAddress, hostName, _port, numCores) {
  ...
  /** Creates and binds the NettyUcxTransportServer, possibly trying multiple ports. */
  private def createServer(bootstraps: List[TransportServerBootstrap], rpcHandler: NettyBlockRpcServer): NettyUcxTransportServer = {
    def startService(port: Int): (NettyUcxTransportServer, Int) = {
      val server = new NettyUcxTransportServer(transportContext, bindAddress, port, rpcHandler, bootstraps.asJava)
