...
class NettyUcxTransportServer(
  ...
  def this(
    ...
    this(context, context.getConf(), appRpcHandler,
  ...
  private def init(hostToBind: String, portToBind: Int) = {
    val bossPoolPrefix = "UCX-shuffle-boss"
    val bossThreadFactory = new DefaultThreadFactory(bossPoolPrefix, true).asInstanceOf[ThreadFactory]
    ...
    bootstrap.childHandler(new ChannelInitializer[SocketChannel]() {
      override protected def initChannel(ch: SocketChannel) = {
        ...
        for (bootstrap <- bootstraps.asScala) {
    ...
    val address = Option(hostToBind) match {
      case Some(_) => new InetSocketAddress(hostToBind, portToBind)
      case None => new InetSocketAddress(portToBind)
    }
